...
/**
 *
 * @author Tanya This is a static class that contains queries that cannot be
 * generated through code due to its complexity
 */
public class Datahelper {
    ...
    public static String selectUsersWithReq = "SELECT * FROM `tbluserrequest` INNER JOIN `tbluser` ON `UserIDFK` = `UserIDPK` INNER JOIN `tblperson` ON `PersonIDPK` = `PersonIDFK` INNER JOIN `tblcontact` ON `ContactIDPK` = `ContactIDFK` GROUP BY `username`";
    ...
    public static String specificUserOrders(String username){
        return "SELECT * FROM `tblorder` INNER JOIN `tbluser` ON `UserIDFK` = `UserIDPK` WHERE `Username` = '"+username+"'";
    }
    ...
    public static String specificUserWithOpenOrder() {
        return "SELECT * FROM `tblorder` INNER JOIN `tbluser` ON `UserIDFK` = `UserIDPK` INNER JOIN `tblPerson` ON `PersonIDPK` = `PersonIDFK` INNER JOIN `tblcontact` ON `ContactIDPK` = `ContactIDFK` WHERE `OrderDate`>`ReceivedDate` ORDER BY `OrderIDPK`";
    }
